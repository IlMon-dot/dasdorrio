...
// Скидка WB по заказам товара — медиана трёх последних покупок, из двух — меньшая. Покупка — корзина (gNumber):
// несколько штук в одном заказе — одна покупка. WB меняет скидку в течение дня (22.09 за двое суток у одного товара
// было 34–45 %), поэтому нужны самые свежие заказы; но у отдельных покупателей скидка выше обычной (последний заказ
// медведя — 45 % при 41 % на витрине), поэтому не один последний. Меньшая из двух — не обещать цену ниже витрины.
function скидкаПоЗаказам(заказы) {
  const покупки = new Map();   // корзина → её самый поздний заказ; заказ без корзины — отдельная покупка
  for (const з of заказы) { const к = з.корзина || з, б = покупки.get(к); if (!б || з.когда > б.когда) покупки.set(к, з); }
  const последние = [...покупки.values()].sort((a, b) => b.когда - a.когда).slice(0, 3);
  const скидки = последние.map((з) => з.спп).sort((a, b) => a - b);
  return { спп: скидки[скидки.length === 3 ? 1 : 0], когда: последние[0].когда, последние: последние.map((з) => з.спп) };
}
...
async function wb() {
  ...
  try {
    ...
    for (const з of заказы || []) {
      ...
      (заказыПо[String(з.nmId)] ||= []).push({ спп: з.spp, когда, корзина: з.gNumber || null });
    }
  } catch (e) {
  ...
  for (const т of список_) {
    ...
    const откуда = свежая ? ', последние покупки ' + свежая.последние.join(', ') + ' %, до ' + дата : ', прошлая, заказы до ' + дата;
