/* Цены площадок для кнопок сайта: node scripts/prices.mjs [--dry]
   Раз в день запускается из .github/workflows/prices.yml и пишет prices.js (--dry — только лог, без записи).
...
catch { console.log('prices-settings.json не прочитан — все площадки включены, регион WB — Москва'); }

let прежние = {};
try { прежние = изФайла('prices.js', 'ЦЕНЫ').товары || {}; }
...
const текст = `/* ЦЕНЫ ПЛОЩАДОК — файл пишет scripts/prices.mjs раз в день, руками не править.
